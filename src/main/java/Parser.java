...
public class Parser {
    ...
    private static MarkCommand checkMark(String fullCommand, TaskList taskList) throws JeanException{
        ...
        } else if (Integer.parseInt(fullCommand.substring(5)) > Task.numberOfTasks || Integer.parseInt(fullCommand.substring(5)) < 1) {
            throw new JeanException("There are only " + Task.numberOfTasks + " task(s)!"
                                    + "\nIl y a seulement " + Task.numberOfTasks + " tâche(s)!");
        } else if (taskList.getTaskList().get(Integer.parseInt(fullCommand.substring(5)) - 1).isDone){
    ...
    private static UnmarkCommand checkUnmark(String fullCommand, TaskList taskList) throws JeanException{
            ...
            } else if (Integer.parseInt(fullCommand.substring(7)) > Task.numberOfTasks || Integer.parseInt(fullCommand.substring(7)) < 1) {
                throw new JeanException("There are only " + Task.numberOfTasks + " task(s)!"
                                        + "\nIl y a seulement " + Task.numberOfTasks + " tâche(s)!");
            } else if (!taskList.getTaskList().get(Integer.parseInt(fullCommand.substring(7)) - 1).isDone){
    ...
    private static DeleteCommand checkDelete(String fullCommand) throws JeanException{
        if (fullCommand.length() == 6) {
            throw new JeanException("You must name a task to delete!"
                                    + "\nNom d'une tâche à supprimer!");
        } else if (Integer.parseInt(fullCommand.substring(7)) < 1 || Integer.parseInt(fullCommand.substring(7)) > Task.numberOfTasks) {
    ...
    public static Command parse(String fullCommand, TaskList taskList) throws JeanException{
        if (fullCommand.equals("bye")) {
            ...
            return checkDelete(fullCommand);
        } else {
