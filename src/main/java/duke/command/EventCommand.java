...
public class EventCommand extends Command {
    ...
    /**
     * A constructor that encapsulates the information of a new event task.
     *
     * @param parameters input of the command, where the taskName and event date will be interpreted.
     */
    public EventCommand(String parameters) {
        super((tasks, ui, storage) -> {
            try {

                String taskName = parameters.split(Event.KEYWORD_TO_SPLIT)[0];
                String at = parameters.split(Event.KEYWORD_TO_SPLIT)[1];
