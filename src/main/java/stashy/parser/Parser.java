...
public class Parser {
    ...
    public static Command parseCommand(String fullCommand) throws StashyException {
        ...
        case FindCommand.KEYWORD:
            return prepareFindCommand(remainingCommand);
        default:
     ...
     */
    private static FindCommand prepareFindCommand(String[] remainingCommand)
            throws StashyException {
        String query = String.join(" ", Arrays.asList(remainingCommand)).strip();
        if (query.isEmpty()) {
