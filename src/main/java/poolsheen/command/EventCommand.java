...
public class EventCommand extends Command{
    ...
    public void execute(TaskList tl, Ui ui, Storage storage) {
        if (rest.isEmpty()) {
            throw new IncompleteCommandException(String.join(" ", rest), "event", "Events need a description and time");
            ...
            if (descE.length() == 0 || timeE.length() == 0) {
                throw new IncompleteCommandException(String.join(" ", rest), "event", "event commands must specify a description and time");
            } else {
