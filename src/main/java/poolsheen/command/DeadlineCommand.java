...
public class DeadlineCommand extends Command {
    ...
    public void execute(TaskList tl, Ui ui, Storage storage) {
        if (rest.isEmpty()) {
            throw new IncompleteCommandException(String.join(" ", rest), "deadline", "Deadlines need a description and time");
        } else if (!rest.contains("/by")) {
            throw new IncompleteCommandException(String.join(" ", rest), "deadline", "deadline commands need a '/by'");
        } else {
            ...
            if (descD.length() == 0 || timeD.length() == 0) {
                throw new IncompleteCommandException(String.join(" ", rest), "deadline", "deadline commands must specify a description and time");
            } else {
